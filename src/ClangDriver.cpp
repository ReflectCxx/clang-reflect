...
namespace clang_reflect
{
    ...
    {
        std::string errMsg;
        clang_reflect::Logger::out("Loading source/header files.");
        auto cdb = clang::tooling::CompilationDatabase::loadFromDirectory(pRootDir, errMsg);
        if (!cdb)
        {
            clang_reflect::Logger::out("CDB not found.");
            clang_reflect::Logger::out("Iterating directory: " + pRootDir);
            FileManager::Instance().loadProjectFilePaths(pRootDir);
        }
        else {
            clang_reflect::Logger::out("CDB loaded from directory: " + pRootDir);
        }
        return cdb;
    }
    ...
    void ClangDriver::compileSourceFiles(const std::string& pRootDir)
    {
        ...
        if (cdb)
        {
            ...
            clang_reflect::Logger::out("Number of source files in CDB: " + std::to_string(srcFiles.size()));
            std::unordered_set<std::string> distinctSrcFiles(srcFiles.begin(), srcFiles.end());
            clang_reflect::Logger::out("Number of distinct source files in CDB: " + std::to_string(distinctSrcFiles.size()));
